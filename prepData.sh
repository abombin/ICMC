...
for i in $(cat newdir.txt); do cp ./original_files/R1/"$i"* ./input/"$i"_R1.fastq.gz; done
for i in $(cat newdir.txt); do cp ./original_files/R2/"$i"* ./input/"$i"_R2.fastq.gz; done
...
for i in $(cat newdir.txt); do mkdir -p process_par/"$i"; cp ./input/"$i"* ./process_par/"$i"/
